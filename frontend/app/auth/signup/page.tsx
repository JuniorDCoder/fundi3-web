...
export default function SignupPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const { error: signUpError } = await signUp(email, password, lang);
    ...
    toast.success(t("toast.accountCreated", lang), {
      description: t("toast.accountCreatedDesc", lang),
      icon: <Mail size={16} className="text-accent" />,
    });

    // Redirect to verify page to tell user to check email
  ...
  return (
    <div className="min-h-screen bg-dark flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-[#4A6358]">
                {t("auth.passwordHint", lang)}
              </p>
            </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  {t("auth.creatingAccount", lang)}
                </>
              ) : (
                t("auth.signup.cta", lang)
              )}
            </button>
          </form>
        ...
        {/* Terms */}
        <p className="text-center text-xs text-[#4A6358] mt-4">
          {t("auth.termsText", lang)}{" "}
