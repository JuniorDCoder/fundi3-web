...
function CourseDetail({ course }: { course: LocalizedCourse }) {
  const { lang } = useLanguage();
  const { user } = useAuth();
  const { summary } = useCourseProgress(course.slug);
  ...
  const fallbackFirstLessonId = course.modules[0]?.lessons[0]?.id ?? null;
  const targetLessonId = summary?.nextLessonId ?? summary?.firstLessonId ?? fallbackFirstLessonId;

  const ctaHref = user
    ? targetLessonId
      ? `/learn/${course.slug}/${targetLessonId}`
      : `/courses/${course.slug}`
    : `/auth/signup?redirect=/courses/${course.slug}`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {course.outcomes.map((outcome, i) => (
              <FadeIn key={`${course.id}-outcome-${i}`} delay={0.05 * i}>
                <div
