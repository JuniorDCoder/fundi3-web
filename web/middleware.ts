...
export async function middleware(request: NextRequest) {
  ...
  // Admin area has its own login page and an extra role check — handled
  // separately from the learner PROTECTED_PREFIXES/AUTH_PREFIXES above.
  if (isAdminArea) {
    if (isAdminLogin) {
      if (user && isAdminUser(user)) {
        const adminHome = request.nextUrl.clone();
        adminHome.pathname = ADMIN_PREFIX;
        adminHome.search = "";
